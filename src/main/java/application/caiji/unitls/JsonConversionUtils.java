...
public class JsonConversionUtils {
	...
	/**
	 * sql结果集的字段名获取,建议名即as后指定的名字如果没有则返回原字段名
	 * key是字段别名,value:字段类型名
	 * 
	 * @return JSONArray 参数为空时返回NUll
	 * @throws SQLException
	 */
	public static JSONObject getColumnlist_v2(RowSet rowset) throws SQLException {
		JSONObject JsonObj=new JSONObject();
		if (rowset != null) {
			ResultSetMetaData rsmd = rowset.getMetaData();
			for (int i = 1; i <= rsmd.getColumnCount(); i++) {
				JsonObj.put(rsmd.getColumnLabel(i),rsmd.getColumnTypeName(i));
			}
			return JsonObj;
		}
		return null;
